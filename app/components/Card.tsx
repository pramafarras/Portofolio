    ...
    return (
        <>
            <div className="w-screen sm:w-full h-full my-6 ">
                <div onClick={() => setModal(true)} className="overflow-hidden relative border-t-2 border-b-2 lg:border-4 sm:border-2 border-secondary aspect-square sm:aspect-video w-screen sm:w-full max-w-[1280px] sm:h-auto h-full mx-auto sm:rounded-lg cursor-pointer group ">
                    ...
                    <div className="absolute inset-1 lg:inset-3 flex items-center justify-between">
                        ...
                        className="rounded-full bg-slate-200/80 hover:bg-slate-300/70 z-10 size-[24px] lg:size-[40px]">
                            <ChevronLeft className="size-[24px] lg:size-[40px]"></ChevronLeft>
                        </button>
                        <button onClick={(e) => {e.stopPropagation(); next();}} className="rounded-full bg-slate-200/80 hover:bg-slate-300/70 z-10 size-[24px] lg:size-[40px]">
                            <ChevronRight className="size-[24px] lg:size-[40px]"></ChevronRight>
                        </button>
                    </div>
                    ...
                    <div className="sm:absolute sm:bottom-0 sm:inset-x-0 bg-primary lg:group-hover:bg-primary sm:h-fit h-1/5 py-0 sm:py-2 transform lg:translate-y-full  lg:group-hover:translate-y-0 lg:transition-all lg:ease-in-out lg:duration-300 lg:p-4 lg:border-t-4 border-t-2 border-secondary flex flex-col content-center items-center justify-center " onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="flex flex-row content-center items-center justify-center gap-1 lg:gap-2 ">
                                <h1 className="font-bold  lg:text-2xl xl:text-4xl text-[14px] sm:text-[18px] md:text-[24px]">Alato</h1>
                            ...
                            <p className="text-[8px] sm:text-[12px] md:text-[14px] lg:text-md lg:mt-2 mt-0 ">Thesis project (Website) that are built to improve physical fitness with filtering features and recommendation system using user-based collaborative filtering</p>
                            <div className="flex flex-wrap lg:gap-2 gap-0.5 mt-0.5 sm:mt-1 lg:mt-2 justify-center items-center content-start w-full">
                                {alatotech.map((value, index) => (
                                    <div key={index} className="rounded-full bg-slate-300 size-fit px-2 py-0.5 mt-1 lg:px-3 lg:py-1 self-center text-center text-[6px] sm:text-[10px] md:text-[14px] lg:text-sm hover:bg-slate-100">
                                    {value}
                                    </div>
                                ))}
                            </div>
